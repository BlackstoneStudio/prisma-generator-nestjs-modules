...
export function getArgumentsApi(type: string, nullable?: boolean) {
  let argument = `{ type: ${type} }`;

  if (nullable) {
    argument = `{ type: ${type}, nullable: true }`;
  }

  return [argument];
}

export function getType(type?: string | undefined, isEnum?: boolean) {
  const args: string[] = [];
...
export function validPassword(fieldName: string) {
  const args: string[] = [];

  if (fieldName.toLocaleLowerCase().includes('password')) {
    args.push('/^(?=.*d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/');
  }
